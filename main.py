...
async def lifespan(app: FastAPI):
    # Start the scheduler when the API boots
    from run_pipeline import start_scheduler, stop_scheduler
    start_scheduler()
    yield
    stop_scheduler()
...
app = FastAPI(
    ...
    description="Technical Challenge Backend — EL KHOBZI Ismail · ESITH Challenge 2026",
    version="2.1.0",
    lifespan=lifespan,
)
...
def generate_report(db: Session = Depends(get_db)):
    ...
    r.append("*Author: EL KHOBZI Ismail · ESITH Technical Challenge 2026*")
...
def scheduler_status():
    """Returns the current state of the automated pipeline scheduler."""
    from run_pipeline import get_status
    return get_status()


@app.post("/scheduler/run")
def scheduler_run_now():
    """Manually trigger a pipeline run immediately (async)."""
    import threading
    from run_pipeline import run_pipeline
    t = threading.Thread(target=run_pipeline, daemon=True)
    t.start()
    return {"message": "Pipeline triggered. Check /scheduler/status for progress."}
